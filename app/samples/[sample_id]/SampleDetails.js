...
// Server-side fetching for initial sample data
// This function will be called on the server to pre-render the component with data
...
export default function SampleDetails({ initialSampleData }) { // Receive initial data as a prop
  ...
  const [sample, setSample] = useState(initialSampleData?.sample || null);
  ...
  useEffect(() => {
    if (!initialSampleData && currentSampleId) { // Only fetch on client if not pre-rendered
...
// Add a `getServerSideProps` or `getStaticProps` function to your page file (e.g., pages/samples/[sample_id].js or app/samples/[sample_id]/page.js for App Router)
// This enables Server-Side Rendering (SSR) for the initial load of sample details.

/*
// Example for pages/samples/[sample_id].js (Pages Router)
export async function getServerSideProps(context) {
  const { sample_id } = context.params;
  const { sample, error } = await getSampleData(sample_id);

  return {
    props: {
      initialSampleData: { sample, error },
    },
  };
}
*/
...
/*
// Example for app/samples/[sample_id]/page.js (App Router)
// In Next.js 13+ App Router, `fetch` is automatically memoized and server-side friendly.
// You would typically fetch data directly within the component for server components,
// or use a separate data fetching layer.
// For client components, you would pass data via props as shown in `SampleDetails({ initialSampleData })`
// and fetch on client for subsequent updates.

// This example is for how you might pass data to a client component in App Router.
// You'd create a server component wrapper:

// app/samples/[sample_id]/page.js
import SampleDetails from '../../components/SampleDetails'; // Adjust path
import { headers } from 'next/headers'; // To pass headers if needed for auth on server

async function getSampleDataServer(sampleId) {
  try {
    const apiBaseUrl = process.env.NEXT_PUBLIC_API_BASE_URL;
    const response = await fetch(`${apiBaseUrl}/api/samples/${sampleId}`, {
      // You might need to pass auth token from cookies or other secure means for server-side fetches
      // headers: { Authorization: `Bearer ${headers().get('cookie')?.split('; ').find(row => row.startsWith('token='))?.split('=')[1]}` },
      cache: 'no-store' // Or 'force-cache' based on your caching strategy
    });
    if (!response.ok) {
